...
export async function createNewLevelDb(id: string) {
    try {
        ...
        if (GlobalVariables.levelDb[pathLevelDb] == undefined) {
            ...
            const claimRevDb = new zidenjsDb.SMTLevelDb(pathLevelDb + `/${levelDbSrc}/revocation`);
        ...
        // const claimRevDb = GlobalVariables.levelDb[pathLevelDb]["claimRevDb"];
        // const authsDb = GlobalVariables.levelDb[pathLevelDb]["authsDb"];
        
        // return {pathLevelDb, claimsDb, claimRevDb, authsDb};
        return pathLevelDb;
    } catch (err: any) {
        throw (err);
    }
...
export async function closeLevelDb(claimsDb: zidenjsDb.SMTLevelDb, claimRevDb: zidenjsDb.SMTLevelDb, authsDb: zidenjsDb.SMTLevelDb) {
    try {
        ...
        // await claimRevDb.nodes.close();
        // await authsDb.nodes.close();
    } catch (err: any) {
        console.log(err);
    }
...
export async function openLevelDb(src: string) {
    ...
    if (GlobalVariables.levelDb[src] == undefined) {
        ...
        const claimRevDb = new zidenjsDb.SMTLevelDb(src + `/${levelDbSrc}/revocation`);
    ...
    // const claimRevDb = GlobalVariables.levelDb[src]["claimRevDb"];
    // const authsDb = GlobalVariables.levelDb[src]["authsDb"];
    // return {claimsDb, claimRevDb, authsDb};
...
export async function open_db(src: string) {
    ...
    GlobalVariables.levelDb[src]["claimRevDb"] = new zidenjsDb.SMTLevelDb(src + `/${levelDbSrc}/revocation`);

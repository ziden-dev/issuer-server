...
import {
  ...
  closeLevelDb,
  restoreDb,
} from "./LevelDbManager.js";
import {
  checkLockTreeState,
  getTreeState,
  saveTreeState,
  saveLastStateTransistion,
} from "./TreeState.js";
...
export async function publishOnly(
...
export async function revokeOnly(signature: SignedChallenge, issuerId: string) {
  const claims = await Claim.find({
    status: ClaimStatus.PENDING_REVOKE,
    issuerId: issuerId,
  });
  const claimIds: Array<string> = claims.map((claim) => {
    return claim.id!;
...
export async function publishAndRevoke(
  signature: SignedChallenge,
  issuerId: string
) {
  const claimsPublish = await Claim.find({
    status: ClaimStatus.PENDING,
    issuerId: issuerId,
  });
  ...
  const claimsRevoke = await Claim.find({
    status: ClaimStatus.PENDING_REVOKE,
    issuerId: issuerId,
  });
  ...
  const currentChallange = await getPublishAndRevokeChallenge(
    claimIdsPublish,
    claimIdsRevoke,
    issuerId
  );
  ...
  const hihv: [ArrayLike<number>, ArrayLike<number>][] = claimsPublish.map(
    (claim) => {
      return [GlobalVariables.F.e(claim.hi!), GlobalVariables.F.e(claim.hv!)];
    }
  );
...
export async function stateTransition(
  issuerId: string,
  signature: SignedChallenge,
  hihvBatch: Array<[ArrayLike<number>, ArrayLike<number>]>,
  revNonces: Array<BigInt>
) {
  ...
  try {
    const stateTransitionInputs =
      await zidenStateTransition.stateTransitionWitnessWithSignatureAndHiHvs(
        signature,
        auth,
        issuerTree,
        [],
    ...
    fs.writeFileSync(
      `${statePath}/${rand}input.json`,
      serializaData(stateTransitionInputs)
    );
    ...
    const out = execSync(
      `snarkjs zkey export soliditycalldata ${statePath}/${rand}public.json ${statePath}/${rand}proof.json`,
      { encoding: "utf-8" }
    )
      .toString()
      .split(",")
    ...
    const stateABI = JSON.parse(
      fs.readFileSync("build/abis/State.json", "utf-8")
    );
    const state = new ethers.Contract(STATE_ADDRESS, stateABI, provider);

    const transitState = await state
      .connect(wallet)
      .functions.transitState(
        publicSig[0],
        publicSig[1],
        publicSig[2],
        publicSig[3],

...
import { ISSUER_PRIVATE_KEY, ISSUER_SERVER_URL, ZIDEN_SERVER_URI } from "../common/config/secrets.js";
...
export async function setupSchemaRegistry() {
    try {
        const schemaJsons = JSON.parse(fs.readFileSync('setup/schema.json', 'utf-8'));
        ...
        for (let i = 0; i < schemaJsons.length; i++) {
            ...
            try {
                ...
                for (let j = 0; j < issuerList.length; j++) {
                    const registry = await SchemaRegistry.findOne({"schemaHash": schema["@hash"], "issuerId": issuerList[i].issuerId});
        ...
        const serviceExisted = (await axios.get(`${ZIDEN_SERVER_URI}/api/v1/services`)).data.services;
